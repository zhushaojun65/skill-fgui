...
模板目录共 28 个文件，覆盖入场、退场、循环、特效、单组件五类常见动画。模板默认显式使用 `frameRate="30"`；FairyGUI 引擎默认帧率是 24fps，若项目要求对齐默认帧率，需要同步调整 `frameRate`、`time` 和 `duration`。
...
- 默认帧率为 `30fps`，模板内通过 `frameRate="30"` 显式声明。
...
|---------|---------------------|-----------|
| 入场补间 `time=0` | ❌ 不需要 | startValue 已在 t=0 生效 |
| 入场补间 `time>0`，startValue ≠ 静止态 | ✅ **必须** | 锁成 startValue（Alpha=0、Scale=0.3 等） |
| 退场补间 `time=0` | ❌ 不需要 | startValue 已在 t=0 生效 |
